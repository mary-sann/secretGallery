...
const computer = {x:1.1316,y:-2.0799,z:1.3502};
let memoGUI;
let camGuide;
let computerGUI;
let computerGUI_size = {width:0.0,height:0.0};
...
async function setup() {
  ...
  geom = await plyToP5geom("../data/room_pc.ply","room");
  ...
  //dom
  //位置座標の初期指定は絶対必要だから最初から全部に書いておいたほうがいい
  //.position()が中で要素を.style = 'absolute'にしてるので
  //.position()を必ず書くか.style('position','absolute'),this.elt.style.position = 'absolute'を書くこと
  ...
  //computer
  computerGUI = createA('../dollHouse/doll.html','見る');
  computerGUI.parent(wrapper);
  computerGUI.position(0,0);
  computerGUI.style('width', 'fit-content');
  computerGUI.style('font-size', ' 16px');
  computerGUI.style('color','white');
  computerGUI.style('font-family', '"Hiragino Sans", sans-serif');
  computerGUI_size.width = computerGUI.size().width;
  computerGUI_size.height = computerGUI.size().height;
...
function draw() {
  ...
  let pc_ndc = screenPosition(computer.x*-100,computer.y*100,computer.z*100);
  computerGUI._translate(width/2+pc_ndc.x-computerGUI_size.width*0.5,
                         height/2+pc_ndc.y-computerGUI_size.height*2.0);
