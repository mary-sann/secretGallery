...
The respective licenses and copyright notices for all external libraries are included directly at 
the beginning of each library file.
...
大手のプラットフォームと比べると集客は弱いですインターネット上ではそれほど悩む要素にはなりません。お金と体力と額の保管場所も要りません。匿名性の保持もできます。女性には安全かもしれません。
...
2026/01/07追記
「サイトについて」「おしらせ」「図録」の項目を追加。
1つのページに１つのhtmlとcssとjsの分離した独立した構造にする。すべて<a>で移動する。
古(いにしえ)の個人サイトはページごとにhtmlが全部分離している構造だったらしい。昔はまだSPAもモジュールも<template>も無かったかららしい。わたしも難しい知識と技術は持っていないので昔のコンピュータ知識ゼロのオタクでも書けたこの構造を見習う。これなら一部の部屋だけ閉鎖したい時にとても楽。部屋の増設も楽！部屋ごとに違うライブラリやエンジンを使っても問題が起こらない。たぶん。
文字やクリックイベントもp5domで作って、canvasは絵の描写だけにする。たぶん軽くなるはず。
今はオブジェクト座標をキャンバス(スクリーン)座標に変換してp5domに送りクリックイベントを作れるコードを書いています。canvas内でクリック判定を計算するより、p5domで作ったhtml要素の方がクリック判定範囲の設定もサイズを変えるだけなので簡単だし絵の描写とは分けられるのでいい気がする、なんとなく。

2026/02/11追記
マウスとタッチ操作でカメラコントロール、説明文を表示するgui追加。
図録ページにprocessingの.pdeのコードを載せられるようにした。
ゲームってどこからがゲームなんだろう。
dat.guiの保存機能がローカルストレージというやつかもしれない。これを使わせてもらって、今後は物語断片探索収集型、フラグ回収で複数の隠し部屋発見分岐を作ってみることにする。物語は暇潰しにスマホのメモアプリにポチポチ書いてるのがあったりする。

2026/05/23　dollhouse追加
背景を作ったらキャラクターも作りたくなります。
なので作ってみました。人間のモデリングははじめてなのでちょっと未熟なつくりのところがあります。改良と練習が必要です。
可愛いゲームの待機画面みたいなものにカスタマイズしていきたいです。
線画をつける方法を思いついたのでやってみました。

＊実装の技術的なお話
３ｄモデル
・今回も頂点カラーのみ
・メイン描写用と線画用グレースケールで別々に色塗りしたもの２体用意
シェーダー
・線画用sobelフィルター
・depthでグラデーションをつけるシェーダー
・メインと線画と合体フィルター
p5.framebuffarというfilter用にぴったりなものが知らない間にできていたので使う。
すごい計算軽い。助かる。

sobelフィルターは他の絵にもいろいろ使っていてお気に入りのアルゴリズムです。
わかりやすいし簡単だし改造しやすい。
これを使えば３ｄモデルに狙った線をつけられるんじゃないかなと思いました。
線をつける方法は、blenderで線画用に複製したモデルをもう一体用意する。
放射マテリアルを、色の値を(1.0)(0.95)(0.9)(0.85)(0.8)...(0.0)と0.05刻みのグレースケールで用意する。
線をつけたい領域ごとに面にマテリアルを割り当てていく。
隣り合った色の差が0.1のところは薄い線、差が0.2、0.3と開くほど濃い線、線をつけたくないところは0.05差でパズルみたいに埋めていく。
sobelフィルターは隣接ピクセルの色の値の差を計っているのでこれで自分で線の場所と太さを制御できる。
sobeのほうで0.05差のところは線を出さないようにちょっと改造する。
おんなじ色だけど奥行きがあるものに線をつけたいときはgl_FragCoord.zでグラデーションをつける。
ただこれだと問題が起こって、gl_FragCoord.zの値はモデルやカメラの位置が変わると変わってしまうので、
特定の位置で、せっかく色分けした他の領域とおんなじ色（値）になってしまって線が消えてしまう部分が出たりする。
どうするかこれから考えます。

＊実機テストとかライブラリの話し
stats.jsというものを導入しはじめました。
three.jsのlibs/の中にもモジュール版が入ってるもので、FPSとMSとMBが測れます。
three.jsの読み込みがめんどくさいので本家のmin.jsのほうを使います。
androidだとブックマークに登録して使う裏技みたいなのがあって簡単でした。
iosはいろいろ試してもうまくいかなかったので、コード全部に組み込んでおくことにしました。

ライブラリ探しが趣味なのですが、guthubの最終更新日が５年、１０年前のものは結構多いです。
processingのライブラリは特に古いものが多いです。
ライセンスをしっかり確認して動くものは動かなくなるまで使うし、書き換えて使えるものは書き換えます。
でも古いライブラリは金銭の絡む動作保証とかしなきゃいけないお仕事では使うべきではないのでしょう。
ちゃんとした事情やエンジニア業界の暗黙のルールはわからないので自分の想像です。
でもわたしは１人で趣味でやっているので許されている範囲でやります。
わたしにとってコードを読むのは読書と同じもので、書いた人のコメントや書き方の癖や、書かれたときの動作環境なんかを考えるのが楽しくて漁っています。
図書館や古本屋で埃をかぶった隅の本を探す遊びとおんなじものです。
疑問で心配事は、プログラマの人にとって最終更新日から５年や１０年もたってるgithubのコードやqiitaの記事を発見して
喜んではしゃいでる人間がいるのってどうなんでしょう、ということです。
そちらの業界の人の心情を聞ける機会があるといいです。
自分だいぶ気持ち悪いかもとたまに我に返ります。
私は昔に描いた絵は、
１未熟で恥ずかしいので見つけ次第処分したい
２今見ても結構自信作なので見つけてくれる人がいて嬉しい
３描いたの忘れてた、まあいいか
みたいなかんじです。
プログラマの人たちはMITで残したライブラリにどんな感情を持っているのでしょうか。
こんな「お宝古本見つけた！」みたいな感じで嬉々として使っていていいんでしょうか。
わたしは完成された古いライブラリ好きなので個人的に限界まで使い続けたいです。
絵は個人主義で絵の所有権も個人のことが多いですが、
プログラマの世界はみんなで歯車を持ち寄って大きなシステムが動き続けるように協力しあっているみたいな印象があります。
自分で書いたコードであっても所有に対する意識にはだいぶ違いがある気がします。

SNSでコード関連のアカウントを持っていると、最近はcodex,claude,copilotなどの話題を見ないということはないです。
わたしはLLMというものに課金をしたことがないので、使ったことはまだありません。
自分の書くコードはとても短い簡単なものなのでいまのところ特に必要になったことはないですが、どんなものなのか知りたいのでいずれ触っていろいろ実験しようと思っています。
最近のチップの性能やどんどん高性能でもっとセキュリティが必要なものが増え続けているのを素人から見ても「人間が手で管理できる範囲を超えてるんだろうなぁ」と思うことが増えてきました。
現代はコードでできたもので生活が成り立っていて、無くなったり止まったり壊れたりすると生活ができなくなるインフラのものがいっぱいあります。
vulkanAPIの5400ページ弱ある全部英語で書かれた仕様書のPDF（しかもバージョンの更新を続けているので見るたびにページ数が増えている）を眺めながら、「もう1人の人間でどうにかできる量ではなくなっているものがいっぱいあるんだろうなぁ」と、vulkanは△ポリゴン１個を画面に出すまで1000行という記事と、最近始めたラズパイ３BとZERO初代とGBAのベアメタルの気軽さと比べながらしみじみ思ったりしています。
macのopenglがもう随分前から非推奨で、processingもjoglからvulkanへの移行の検討の議論をgithubでしているのを見ながらそんなことを考えています。
最近のGPUが高性能になりすぎたからだ、たぶん。
でもLLMってなんなのか仕組みには興味があるので、vulkanの仕様書の日本語翻訳と索引作成特化のローカルLLMを作りたいなと考えています。
LLMは言語の細かい仕様の質問に使うときがあります。 
webのdomの構造はどういう風に管理されているとか、ｃ言語のコンパイラは誰が作ってきたとか、javaが実行されるまでにclassがどういうふうに整理されているとか、エントリーポイントがある言語とない言語の実行の順番の仕組みとか、パソコン内でOSとソフトウェアとブラウザにはそれぞれどのくらいメモリが割り当てられているのかとか、自分がもしエンジニアという職種だったり工学科だったら、先輩や先生や上司に質問できるような教科書に決まった答えが書いてある類いの索引としてとても便利です。
近々、パソコンを新調する予定なのでローカルLLMを試してみようと思います。
何事も自分でやっていないとわかりません。LLMについても知る機会になります。
やってみて使うか使わないかは、作った後に自分で考えて決めます。
選択肢がたくさん増えた中で何を選ぶのか、（選ぶことをやめることを選ぶということすら）選択権を自分１人で持てるのがお仕事と違う１人で制作をする楽しさのひとつかな思います。
